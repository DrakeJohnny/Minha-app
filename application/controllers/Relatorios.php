...
class Relatorios extends CI_Controller {
    ...
    public function vendas() {
        ...
        if ($data_inicial) {
            ...
            if ($this->vendas_model->gerar_relatorios_vendas($data_inicial, $data_final)) {

                $empresa = $this->core_model->get_by_id('sistema', array('sistema_id' => 1));

                $vendas = $this->vendas_model->gerar_relatorios_vendas($data_inicial, $data_final);
    ...
    public function os() {
        ...
        if ($data_inicial) {
            ...
            if ($this->ordem_servicos_model->gerar_relatorios_os($data_inicial, $data_final)) {

                $empresa = $this->core_model->get_by_id('sistema', array('sistema_id' => 1));

                $ordens_servicos = $this->ordem_servicos_model->gerar_relatorios_os($data_inicial, $data_final);

                $file_name = 'Relatório de ordens de serviço';
                ...
                $html .= '<th>Ordem de serviço ID</th>';
                $html .= '<th>Data</th>';
                $html .= '<th>Cliente</th>';
                $html .= '<th>Forma de pagamento</th>';
                $html .= '<th>Valor total</th>';
